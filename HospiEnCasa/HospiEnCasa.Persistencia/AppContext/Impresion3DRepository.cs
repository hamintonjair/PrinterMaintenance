      ...
      //instancia
       ...
       //buscar por nombre
        ...
        IEnumerable<Impresiones3D> IImpresion3DRepository.FindMultipleParameter(string value){
            return _context.impresion3D.Where(p => p.cliente.Contains(value) || p.Tipo_impresion.Contains(value) 
            || p.cantidad.Contains(value) || p.precio.Contains(value));
        }
        ...
        IEnumerable<Impresiones3D> IImpresion3DRepository.Buscador(string busqueda){
            return _context.impresion3D.Where(p => p.cliente.Contains(busqueda) || p.Tipo_impresion.Contains(busqueda)
             || p.cantidad.Contains(busqueda) || p.precio.Contains(busqueda));
